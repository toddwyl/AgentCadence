...
export function AutoPlannerDialog() {
  ...
  const close = () => setShowAutoPlanner(false);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center theme-backdrop backdrop-blur-sm animate-fade-in">
      <div className="w-full max-w-2xl glass-panel-strong shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid var(--color-border)' }}>
          <div className="flex items-center gap-3">
            <div
              ...
              style={{ background: 'var(--color-active-bg)', border: '1px solid var(--color-accent-border)' }}
            >
              <svg className="w-4 h-4 theme-accent-text" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z" /></svg>
            </div>
            <div><h2 className="text-base font-semibold theme-text text-balance">{t.planner.title}</h2><p className="text-xs theme-text-secondary text-pretty">{t.planner.subtitle}</p></div>
          </div>
        ...
        <div className="p-6 space-y-4">
            ...
            <div className="space-y-2 animate-fade-in">{PHASES.map((phase, idx) => {
              ...
              return (
                <div key={phase} className="flex items-center gap-3">
                  ...
                  : isActive ? <div className="w-5 h-5 flex items-center justify-center"><div className="w-3.5 h-3.5 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: 'var(--color-accent-solid)', borderTopColor: 'transparent' }} /></div>
          ...
          {isPlanningInProgress && planningLogs && (<div><label className="block text-xs theme-text-secondary mb-1.5">{t.planner.agentOutput}</label><pre className="p-3 theme-bg-0 rounded-lg text-xs font-mono theme-text-secondary max-h-[150px] overflow-auto whitespace-pre-wrap">{planningLogs}</pre></div>)}
        ...
        <div className="flex justify-end gap-3 px-6 py-4" style={{ borderTop: '1px solid var(--color-border)' }}>
          <button onClick={close} className="btn-ghost text-sm">{t.sidebar.cancel}</button>
