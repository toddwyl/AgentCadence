...
export function Header() {
  const {
    ...
    showFlowchart, toggleFlowchart, showMonitor, toggleMonitor,
    ...
    <header className="h-14 px-6 flex items-center justify-between theme-header backdrop-blur-sm">
      <div className="flex items-center gap-4 min-w-0 flex-1">
        <div className="min-w-0">
          <h2 className="text-base font-semibold theme-text text-balance">{pipeline.name}</h2>
          <div className="flex items-center gap-2 mt-1 max-w-[min(100%,480px)]">
            <span className="text-xs theme-text-tertiary shrink-0">{t.header.workingDirLabel}</span>
            <input
              className="input-field text-xs font-mono py-1.5 px-2 flex-1 min-w-0"
              value={wd}
      ...
      <div className="flex items-center gap-3 shrink-0">
        <button onClick={toggleMonitor}
          className={`btn-ghost text-xs flex items-center gap-1.5 ${showMonitor ? 'theme-text theme-active-bg' : ''}`}
          style={showMonitor ? { border: '1px solid var(--color-accent-border)' } : undefined}>
          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M6 20.25h12m-7.5-3v3m3-3v3m-9-12V6a2.25 2.25 0 012.25-2.25h12A2.25 2.25 0 0120.25 6v8.25m-16.5 0h16.5" />
          </svg>
          {t.header.monitor}
        </button>
        <button onClick={toggleFlowchart}
          className={`btn-ghost text-xs flex items-center gap-1.5 ${showFlowchart ? 'theme-text theme-active-bg' : ''}`}
          style={showFlowchart ? { border: '1px solid var(--color-accent-border)' } : undefined}>
