      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        <div className="flex items-center justify-end">
          <button
            ...
            className="btn-ghost text-sm flex items-center gap-2 px-3 py-2 rounded-lg"
            title={t.editor.pipelineSettings}
            aria-label={t.editor.pipelineSettings}
          >
            ...
            <span>{t.editor.pipelineSettings}</span>
          </button>
        </div>
        {pipeline.stages.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center animate-fade-in">
          <div className="size-16 rounded-2xl theme-bg-2 flex items-center justify-center mb-4">
        ...
        {addingStage ? (
          <div className="glass-panel p-4 space-y-3 animate-fade-in">
            ...
            <div className="flex gap-2">
              <button onClick={handleAddStage} className="btn-primary text-sm">{t.editor.addStage}</button>
              <button onClick={() => setAddingStage(false)} className="btn-ghost text-sm">{t.sidebar.cancel}</button>
            </div>
          </div>
        ) : pipeline.stages.length > 0 && (
          <button onClick={() => setAddingStage(true)} className="w-full py-3 border-2 border-dashed rounded-xl theme-text-muted hover:theme-text-secondary transition-all text-base flex items-center justify-center gap-2" style={{ borderColor: 'var(--color-border)' }}>
    ...
    <div className="glass-panel overflow-hidden animate-slide-in">
      <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid var(--color-border)' }}>
        <div className="flex items-center gap-3">
          <span className="text-xs font-mono theme-text-muted tabular-nums w-6 text-right" title={`#${stageIndex + 1}`}>
            {stageIndex + 1}
          </span>
          <h3 className="text-lg font-semibold theme-text text-balance">{stage.name}</h3>
          <span className={`badge text-xs ${stage.executionMode === 'parallel' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-amber-500/10 text-amber-500'}`}>
            {stage.executionMode === 'parallel' ? `⇉ ${t.editor.parallel}` : `→ ${t.editor.sequential}`}
          </span>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => updateStage(pipelineId, stage.id, { executionMode: stage.executionMode === 'parallel' ? 'sequential' : 'parallel' })} className="btn-ghost text-xs theme-text-secondary">{t.editor.toggleMode}</button>
          <button onClick={() => deleteStage(pipelineId, stage.id)} className="btn-ghost text-xs text-red-400/70 hover:text-red-500">{t.editor.delete}</button>
        </div>
      </div>
      <div className="p-3 space-y-1.5">
        ...
        {addingStep ? (
          <div className="p-3 theme-bg-0 rounded-lg space-y-2 animate-fade-in" style={{ opacity: 0.9 }}>
            ...
            <div className="flex gap-2">
              <button onClick={handleAddStep} className="btn-primary text-sm">{t.editor.addStep}</button>
              <button onClick={() => setAddingStep(false)} className="btn-ghost text-sm">{t.sidebar.cancel}</button>
            </div>
          </div>
        ) : (
          <button onClick={() => setAddingStep(true)} className="w-full py-2.5 text-sm theme-text-muted theme-hover rounded-lg transition-all flex items-center justify-center gap-2">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>
            {t.editor.addStep}
          </button>
        )}
...
function StepRow({ step, stepIndex, selected, onClick }: { step: PipelineStep; stepIndex: number; selected: boolean; onClick: () => void; }) {
  ...
  return (
    <div onClick={onClick}
      ...
      <div className="size-8 rounded-lg flex items-center justify-center text-sm font-bold tabular-nums" style={{ backgroundColor: meta.tintColor + '18', color: meta.tintColor }}>{stepIndex + 1}</div>
      <div className="flex-1 min-w-0">
        <div className="text-sm leading-tight theme-text-secondary truncate text-balance">{step.name}</div>
        <div className="text-xs theme-text-muted truncate mt-1 text-pretty">{step.prompt.slice(0, 80)}</div>
      </div>
      <span className="text-xs font-medium px-3 py-1 rounded-full" style={{ backgroundColor: meta.tintColor + '15', color: meta.tintColor }}>{meta.displayName}</span>
    </div>
  );
}
